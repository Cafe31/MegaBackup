...
def download_from_aws(credentials, target_path, backup_path, backup_name):
    s3 = boto3.client('s3', aws_access_key_id=credentials['access_key'],
                        aws_secret_access_key=credentials['secret_key'])

    try:
        s3.download_file(backup_path + backup_name, credentials['bucket'], target_path + backup_name)
